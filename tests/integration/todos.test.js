...
async function getTodoById(requiredId, token) {
  const getTodoResponse = await chai
...
async function getAllTodos(token) {
  const getAllTodoResponse = await chai
    .request(baseUrl.local.SERVER_URL)
...
async function createTodoAndGetId(token) {
  const filePath = path.join(
    ...
    testTodoDatas[1].image.split("/")[3],
    testTodoDatas[1].image.split("/")[4]
  );
  ...
  const newlyCreatedTodo = await getTodoById(
    createTodoResponse.body.todoId,
    token
  );
...
describe("fetchingTodoById", () => {
  describe("test cases will be passed in the below scenarios", () => {
    ...
    it("should give the expected title", async () => {
      const getTodoWithImageResponse = await getTodoById(
        todoWithImageId,
        token
      );
      const getTodoWithoutImageResponse = await getTodoById(
        todoWithoutImageId,
        token
      );
      expect(getTodoWithImageResponse.status).to.equal(200);
      expect(getTodoWithImageResponse.body.todo.taskName).to.equal(
        testTodoDatas[1].taskName
      );
      expect(getTodoWithImageResponse.body.todo.isCompleted).to.be.false;
      expect(getTodoWithoutImageResponse.status).to.equal(200);
      expect(getTodoWithoutImageResponse.body.todo.taskName).to.equal(
        testTodoDatas[0].taskName
      );
    ...
    it("should give the expected image", async () => {
      ...
      expect(getTodoResponse.body.todo.taskName).to.equal(
        testTodoDatas[1].taskName
      );
      const fileDownloadPath = path.join(
        "tests",
        "uploads",
        getTodoResponse.body.todo.referencePath.split("/")[2]
      );
    ...
    it("should throw if it cannot find that particular id", async () => {
      ...
      expect(deleteTodoResponse.body.msg).to.equal(
      ...
      for (const todoData of testTodoDatas) {
        if (todoData.image !== undefined) {
          ...
          const fileDownloadPath = path.join(
            "tests",
            "uploads",
            newTodo.body.todo.referencePath.split("/")[2]
          );
          await checkForUploadedImg(
            fileDownloadPath,
            path.join("utils", "assets", "img-1.jpg"),
            newTodo.body.todo.referencePath
          );
...
describe("updateTodoById", () => {
  describe("test cases will be passed in the below scenarios", async () => {
    ...
    it("should have correct data when taskname is updated while image is not updated", async () => {
      const updateTodoResponse = await chai
        .request(baseUrl.local.SERVER_URL)
      ...
      const getUpdatedTodo = await getTodoById(
        newlyCreatedTodo.body.todo.id,
        token
      );
    ...
    it("should have correct data when image is updated while title is not updated", async () => {
      ...
      const fileDownloadPath = path.join(
        "tests",
        "uploads",
        getUpdatedTodo.body.todo.referencePath.split("/")[2]
      );
      await checkForUploadedImg(
        fileDownloadPath,
        filePath,
        getUpdatedTodo.body.todo.referencePath
      );
    });
    it("form validation on entering invalid fields", async () => {
      const payloadData = [
        {
          key: "id",
          wrongValues: ["", 123456],
          correctValue: newlyCreatedTodo.body.todo.id
        },
    ...
    it("throw error if token is not passed", async () => {
      const tokenValidationPayload = {
        ...
        url: "/api/todos/updateTodo",
        payload: {
          taskName: testTodoDatas[0].taskName,
          id: newlyCreatedTodo.body.todo.id
        }
      };
      await checkIfTokenPassed(tokenValidationPayload);
    });
