...
describe("signUpApiPrep", () => {
  describe("when signup is called", () => {
    before(async () => {
      const existingUser = await read.singleByKey("users", {
...
describe("loginApiPrep", () => {
  describe("when login is called", () => {
    ...
    before(async () => {
      ...
      expect(res.body.msg).to.be.oneOf([
    ...
    it(`it should see whether the username already exists and
      ...
      const findEmail = await read.singleByKey("users", {
        email: userDetails.email
      });
...
describe("deleteApiPrep", () => {
  describe("when delete api is called", () => {
    ...
    it("respond with status code 200 when email, password,token is passed", async () => {
      const res = await chai
        .request(baseUrl.local.SERVER_URL)
        .delete("/api/auth/deleteUser")
        .set({ Authorization: `Bearer ${token}` })
    ...
    afterEach(async () => {
      const userExists = await read.singleByKey("users", {
        email: "BrandonFlynn12@gmail.com"
      });
      if (userExists) {
        const res = await chai
